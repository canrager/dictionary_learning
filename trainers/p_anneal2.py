...
class PAnneal2Trainer(SAETrainer):
    """
    SAE training scheme with the option to anneal the sparsity parameter p.
    You can further choose to use Lp or Lp^p sparsity.
    Difference from PAnnealTrainer: keeps constant local gradient of sparsity loss instead of constant local sparsity loss
    """
    def __init__(self, 
    ...
    def lp_norm_grad(self, f, p):
        if self.sparsity_function == 'Lp^p':
            pow_f = f.pow(p-1)
            pow_f[f <= 0] = 0
            return p * pow_f.sum(dim=-1).mean()
        elif self.sparsity_function == 'Lp':
            pow_f = f.pow(p-1)
            pow_f[f <= 0] = 0
            return (pow_f.sum(dim=-1) * f.pow(p).sum(dim=-1)).mean()
        else:
            raise ValueError("Sparsity function must be 'Lp' or 'Lp^p'")
    ...
    def loss(self, x, step, logging=False):
        ...
        if self.next_p is not None:
            lp_loss_grad = self.lp_norm_grad(f, self.p)
            lp_loss_grad_next = self.lp_norm_grad(f, self.next_p)
            self.sparsity_queue.append([lp_loss_grad.item(), lp_loss_grad_next.item()])
